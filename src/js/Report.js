...
Tarpo.Report.show = function(listId){
    ...
    var report_data = {
        ...
        notes: Tarpo.Db.queryScalar('select count(*) from visit where type="Note"' + xF),
        notes_house_unoccupied: Tarpo.Db.queryScalar('select count(*) from visit where type="Note - House unoccupied"' + xF),
        notes_people_away: Tarpo.Db.queryScalar('select count(*) from visit where type="Note - People away"' + xF),
        notes_no_dogs: Tarpo.Db.queryScalar('select count(*) from visit where type="Note - No dogs"' + xF),
