...
Ext.Template.prototype.compile = function(){
    ...
    var fn = function(m, name, format, args, offset, s){
        if (prevOffset != offset) {
            var action = {
                type: 1,
                value: s.substr(prevOffset, offset - prevOffset)
            };
            arr.push(action);
        }
    ...
    if (prevOffset != (s.length - 1)) {
        var action = {
            type: 1,
            value: s.substr(prevOffset, s.length - prevOffset)
        };
        arr.push(action);
    }
...
Ext.ColorPalette.prototype.tpl = new Ext.XTemplate('<tpl for="."><a href="#" class="color-{.}" hidefocus="on"><em><span style="background:#{.}" unselectable="on">&#160;</span></em></a></tpl>');
Ext.override(Ext.grid.GroupingView, {
    startGroup: new Ext.XTemplate('<div id="{groupId}" class="x-grid-group {cls}">', '<div id="{groupId}-hd" class="x-grid-group-hd" style="{style}"><div>', this.groupTextTpl, '</div></div>', '<div id="{groupId}-bd" class="x-grid-group-body">')
});
...
Ext.air.NativeWindow = function(config){
    ...
    this.addEvents('close', 'closing', 'move', 'moving', 'resize', 'resizing', 'displayStateChange', 'displayStateChanging');
...
Ext.air.MusicPlayer = Ext.extend(Ext.util.Observable, {
    ...
    constructor: function(config){
        config = config || {};
        Ext.apply(this, config);
        this.addEvents('stop', 'pause', 'play', 'load', 'id3info', 'complete', 'progress', 'skip');
